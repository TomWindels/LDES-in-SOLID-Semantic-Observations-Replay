...
import org.lib.ldests.rdf.*
import org.lib.ldests.rdf.ontology.*
import org.lib.ldests.rdf.ontology.LDESTS
import org.lib.ldests.util.error
...
SELECT ?shapeTarget ?propType ?propPath ?propKind ?propDType ?propDefValue ?propValues ?propStart ?propEnd WHERE {
    ...
    ?shapeProperty <${SHACL.path.value}> ?propPath .
    OPTIONAL {
        ?shapeProperty <${SHACL.nodeKind.value}> ?propKind .
    }
    OPTIONAL {
        ?shapeProperty <${SHAPETS.constantValue.value}> ?propDefValue .
    }
    OPTIONAL {
        ?shapeProperty <${SHAPETS.constantValues.value}>/rdf:rest*/rdf:first ?propValues .
    }
    OPTIONAL {
        ?shapeProperty <${SHACL.dataType.value}> ?propDType .
    }
    OPTIONAL {
        ?shapeProperty <${SHAPETS.startIndex.value}> ?propStart .
        ?shapeProperty <${SHAPETS.endIndex.value}> ?propEnd .
    }
}
""")
        ...
        query(SHAPE_SPARQL_QUERY_FOR_STREAM(stream)) { binding ->
            ...
            when (binding["propType"]!!.value) {
                ...
                SHAPETS.Variable.value -> {
                    vars[binding["propPath"]!! as NamedNodeTerm] =
                        Shape.VariableProperty(
